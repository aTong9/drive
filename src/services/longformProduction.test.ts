...
import {
  documentary90Chapters,
  documentary90Targets,
  documentaryLessons,
} from "../data/documentaryTutorial.js";
import {
  longformBlueprints,
...
test("longform guide covers documentary, cinematic and observational formats", () => {
  assert.deepEqual(
    longformFormats.map((item) => item.id),
    [
      "documentary",
      "cinematic",
      "observational",
...
test("longform production covers the full lifecycle", () => {
  ...
  for (const phase of longformPhases)
    assert.ok(phase.tasks.length >= 3 && phase.checks.length >= 3);
});
test("longform guide includes editing risks and corrective action", () => {
  assert.ok(longformEditWarnings.length >= 12);
  assert.ok(longformEditWarnings.some(([problem]) => problem === "版本失控"));
  for (const item of longformEditWarnings)
    assert.ok(item[0] && item[1] && item[2]);
});
test("every longform format has an actionable production blueprint", () => {
  assert.deepEqual(
    longformBlueprints.map((item) => item.formatId),
    longformFormats.map((item) => item.id),
  );
...
test("release quality gates cover the complete delivery chain", () => {
  assert.deepEqual(
    longformQualityGates.map((gate) => gate.category),
    [
      "故事与结构",
      "事实与伦理",
      "画面与连续性",
  ...
  assert.ok(
    longformQualityGates.reduce(
      (total, gate) => total + gate.items.length,
      0,
    ) >= 35,
  );
...
test("the documentary tutorial is a complete 90 minute production course", () => {
  ...
  assert.deepEqual(
    documentaryLessons.map((lesson) => lesson.id),
    [
      "premise",
      "characters",
      "preproduction",
  ...
  for (const lesson of documentaryLessons) {
    ...
    for (const step of lesson.steps)
      assert.ok(step.detail && step.example && step.check);
  }
});
