...
function formatDrivingTime(seconds: number) {
  ...
  return hours
    ? `${hours}小时${remainder ? `${remainder}分` : ""}`
    : `${minutes}分钟`;
}
...
export function RouteDetail({
  ...
  const accessOnlyLabel = usesWgs84
    ? "公共交通 · 步行连接"
    : "步行景区 · 导航锚点";
  const accessOnlyMessage = usesWgs84
    ? "展示真实地点与行程顺序，不调用高德大陆驾车规划"
    : "仅展示入口与景区范围，不生成景区内驾车路线";
    ...
    <aside className="detail-panel">
        ...
        <div className="detail-hero-copy">
          ...
          <div>
            ...
            <span>
              <Navigation size={14} />{" "}
              {drivingSummary?.status === "ready"
                ? `${(drivingSummary.distanceMeters / 1000).toFixed(1)} 公里`
                : `${waypoints.length} 个${driveOnly ? "道路锚点" : "拍摄点"}`}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="detail-scroll">
        <section className="condition-grid">
          ...
          <div>
            ...
            <small>推荐时段</small>
            <strong>
              {route.best.times.map((item) => timeLabels[item]).join(" · ")}
            </strong>
          </div>
        </section>

        <section
          className={`driving-result ${drivingSummary?.status ?? "idle"}`}
          aria-live="polite"
        >
          <div>
            {drivingSummary?.status === "access-only" ? (
              <Trees size={16} />
            ) : (
              <Navigation size={16} />
            )}
            <span>
              ...
              <strong>
                {drivingSummary?.status === "access-only"
                  ? accessOnlyMessage
                  : drivingSummary?.status === "ready"
                    ? `${(drivingSummary.distanceMeters / 1000).toFixed(1)} 公里 · ${formatDrivingTime(drivingSummary.durationSeconds)}`
                    : drivingSummary?.status === "error"
                      ? drivingSummary.message
                      : "正在计算道路与通行时间…"}
              </strong>
            </span>
          </div>
          {drivingSummary?.status === "ready" && (
            <small>
              {drivingSummary.tollsYuan > 0
                ? `预计收费 ¥${drivingSummary.tollsYuan}`
                : "预计无收费"}
              {drivingSummary.hasRestriction
                ? " · 存在无法规避的限行路段"
                : " · 已规避限行"}
            </small>
          )}
        ...
        <section className="detail-section">
          <div className="section-title">
            <div>
              <p>ITINERARY</p>
              <h3>{driveOnly ? "连续驾车行程" : "拍摄行程"}</h3>
            </div>
            <span>
              {waypoints.length} {driveOnly ? "WAYPOINTS" : "STOPS"}
            </span>
          </div>
        ...
        <section className="detail-section">
          ...
          {cameraPresets.map((preset) => (
            <div className="camera-preset" key={preset.id}>
              ...
              <div>
                <strong>{preset.camera}</strong>
                <small>{preset.scene.replaceAll("-", " ")}</small>
              </div>
              <div className="camera-values">
                <span>{preset.settings.resolution}</span>
                <span>{preset.settings.fps} FPS</span>
                <span>
                  ISO {preset.settings.iso.min}–{preset.settings.iso.max}
                </span>
              </div>
            </div>
          ))}
        ...
        {!!route.verification.sources?.length && (
          <section className="detail-section">
            <div className="section-title">
              <div>
            ...
            {route.verification.sources.map((source) => (
              <a
      ...
      <div className="detail-action">
        <div className="detail-action-buttons">
          <button
            className={`detail-plan-button ${planned ? "is-planned" : ""}`}
            onClick={() => (planned ? setView("plans") : setDialogOpen(true))}
          >
          ...
          {amapNavigationUrl && (
            <a
              className="detail-amap-button"
              href={amapNavigationUrl}
              target="_blank"
              rel="noreferrer"
              aria-label={`在高德地图打开：${route.name}`}
            >
              <ExternalLink size={18} />
              高德地图打开
            </a>
        ...
        <small>
          {shareStatus === "error"
            ? "无法调用分享或剪贴板，请检查浏览器权限"
            : amapNavigationUrl && driveOnly && waypoints.length > 3
              ? "高德支持 1 个途经点 · 完整锚点与拍摄说明请保留本站分享链接"
              : "路线仍需实地核验 · 出发前查看实时路况"}
        </small>
      </div>
