...
const captureDescriptions = {
  "scenic-drive": "真实道路 · 长距离转场",
  "rain-walk": "停车进入 · 慢行取景",
  "stationary-nature": "固定机位 · 自然收音",
} as const;
const captureIcons = {
  "scenic-drive": CarFront,
  "rain-walk": CloudRain,
  "stationary-nature": Trees,
} as const;
...
export function LocationView({
  ...
  const handleImport = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    try {
      const checks = await readFieldChecks(
        file,
        catalogSchemaVersion,
        locations,
      );
    ...
    <main className={`location-page mode-${browseMode}`}>
      <section className="location-browser">
        <header className="location-head">
          ...
          <div className="location-head-actions">
            <div className="location-head-stats">
              <strong>{coveredProvinceCount}/34</strong>
              <small>实景省级覆盖</small>
              <strong>{administrativeDivisionCount}</strong>
              <small>城市与区域</small>
            ...
            />
            <button
              className="import-checks"
              onClick={() => importInputRef.current?.click()}
            >
              <Download size={15} /> 导入核验
            </button>
            <button
        ...
        >
          <div className="region-path">
            ...
            {region.city && (
              <>
                <ChevronRight size={12} />
                <strong>{divisionLabel(region.province, region.city)}</strong>
              </>
            )}
          </div>
        ...
        <div className="library-switch" role="tablist" aria-label="浏览内容">
          <button
            role="tab"
            aria-selected={browseMode === "locations"}
            onClick={() => setBrowseMode("locations")}
          >
            <MapPin size={16} />
            <span>地点</span>
            <small>{filtered.length} 个可用地点</small>
          </button>
          <button
            role="tab"
        ...
        {browseMode === "locations" ? (
          <>
            <div className="location-filters">
              ...
              {[...new Set(locations.map((location) => location.type))].map(
                (item) => (
                  <button
                    key={item}
            ...
            <div className="location-grid">
              {pagedLocations.items.map((location) => {
                ...
                return (
                  <button
                    key={location.id}
                    className={`location-card ${selected.id === location.id ? "active" : ""}`}
                  ...
                  >
                    ...
                    <div>
                      <span>
                        {typeLabels[location.type]} · {location.city}
                      </span>
                      <h2>{location.name}</h2>
                      <small>
                        <Footprints size={12} />{" "}
            ...
            <LocalPaginationControls
              {...pagedLocations}
              onPageChange={(page) => changePage(page, setLocationPage)}
            />
          </>
        ) : (
          <>
            <div className="capture-library-filters">
              <button
                className={captureStyle === "all" && !driveOnly ? "active" : ""}
                onClick={() => {
                  setCaptureStyle("all");
                  setDriveOnly(false);
                }}
              >
                ...
                <span>
                  <strong>全部路线</strong>
                  <small>查看所有创作流程</small>
                </span>
              </button>
              {(
                ["scenic-drive", "rain-walk", "stationary-nature"] as const
              ).map((style) => {
                const Icon = captureIcons[style];
                return (
                  <button
                    key={style}
                    className={`${captureStyle === style && !driveOnly ? "active" : ""} style-${style}`}
                    onClick={() => {
                  ...
                  >
                    <Icon size={18} />
                    <span>
                      <strong>{captureLabels[style]}</strong>
                      <small>{captureDescriptions[style]}</small>
                    </span>
                  </button>
                );
              })}
              <button
                className={`drive-only-library-filter ${driveOnly ? "active" : ""}`}
                aria-pressed={driveOnly}
                onClick={() => {
                  setDriveOnly(!driveOnly);
                  setCaptureStyle(!driveOnly ? "scenic-drive" : "all");
                }}
              >
                <CarFront size={18} />
                <span>
            ...
            <div className="library-route-grid">
              {pagedRoutes.items.map(({ route, waypoints }) => {
                const Icon = captureIcons[route.captureStyle];
                const routeDriveOnly = route.executionMode === "drive-only";
                return (
                  <article
                    className={`library-route-card style-${route.captureStyle}`}
                    key={route.id}
                  >
                    <GeoPhotoThumbnail
                      id={route.id}
                      ...
                      points={waypoints}
                    />
                    <div className="library-route-top">
                      <span>
                        <Icon size={16} />{" "}
                        {routeDriveOnly
                          ? "纯驾车 · 无需下车"
                          : captureLabels[route.captureStyle]}
                      </span>
                      <small>
                        <ShieldCheck size={12} />{" "}
                    ...
                    <div className="library-route-meta">
                      ...
                      <span>
                        ...
                        {routeDriveOnly ? "道路锚点" : "拍摄点"}
                      </span>
                      <span>{route.cities.join(" · ")}</span>
                    </div>
                    <ol>
        ...
        {browseMode === "locations" && filtered.length === 0 && (
          <div className="library-empty">
            ...
            <span>拍摄地点与路线仍待来源核验，暂无虚构占位数据</span>
          </div>
        )}
      </section>
      ...
      >
        <div className="location-detail-hero">
          ...
          <h2>{selected.name}</h2>
          <p>
            <MapPin size={14} /> {selected.province} · {selected.city} ·{" "}
        ...
        <div className="location-detail-scroll">
          ...
          <section>
            ...
            <h3>声音环境</h3>
            <div className="sound-environment">
              <div>
                <AudioLines size={17} />
                <span>
                  <small>主要声景</small>
          ...
          <section>
            ...
            {check && !editing ? (
              <div className="field-check-card">
                <span>
                  <CheckCircle2 size={15} /> {check.visitedAt} 已实地核验
                </span>
                <dl>
                  <div>
                    <dt>停车</dt>
                    <dd>{check.parkingNote || "未记录"}</dd>
                  </div>
                  <div>
                    <dt>光线</dt>
                    <dd>{check.lightNote || "未记录"}</dd>
                  </div>
                  <div>
                    <dt>声音</dt>
                    <dd>{check.soundNote || "未记录"}</dd>
                  </div>
                </dl>
              ...
              <form className="field-check-form" onSubmit={submitCheck}>
                <label>
                  核验日期
                  <input
                    type="date"
                    ...
                    onChange={(event) =>
                      setDraft({ ...draft, visitedAt: event.target.value })
                    }
                    required
                  />
                </label>
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setDraft({ ...draft, parkingNote: event.target.value })
                    }
                    placeholder="例如：1号停车场，工作日有空位"
                  />
                </label>
                <label>
                  现场光线
                  <input
                    value={draft.lightNote}
                    onChange={(event) =>
                      setDraft({ ...draft, lightNote: event.target.value })
                    }
                    placeholder="例如：蓝调时刻建筑灯光稳定"
                  />
                </label>
                <label>
                  声音环境
                  <input
                    value={draft.soundNote}
                    onChange={(event) =>
                      setDraft({ ...draft, soundNote: event.target.value })
                    }
                    placeholder="例如：海浪清晰，偶有车辆噪声"
                  />
                </label>
                <label>
                  综合备注
                  <textarea
                    ...
                    onChange={(event) =>
                      setDraft({ ...draft, overallNote: event.target.value })
                    }
                    rows={3}
                    placeholder="记录机位、限制和下次改进"
                    required
                  />
                </label>
                <div>
                  ...
                  <button type="submit">保存核验</button>
                </div>
              </form>
            ) : (
              <button
          ...
          {relatedRoutes.length > 0 && (
            <section>
              ...
              {relatedRoutes.map((route) => (
                <button
                  className="related-route"
                  key={route.route.id}
                  onClick={() => selectRoute(route.route.id)}
                >
                  <Navigation size={15} />
