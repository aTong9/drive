...
export function PostWorkflowView({ workflow, routes }: { workflow: DavinciWorkflow; routes: ResolvedRoute[] }) {
  ...
  const selectedStageIndex = workflow.stages.findIndex((stage) => stage.id === selected?.id);
  const selectedStageTasks = selected ? tasksByStage.get(selected.id) ?? [] : [];
  const selectedStageCompleted = selectedStageTasks.filter((task) => task.completed).length;
  const stageIsDone = (stageId: DavinciStageId) => { const tasks = tasksByStage.get(stageId) ?? []; return tasks.length > 0 && tasks.every((task) => task.completed); };
  ...
  return <main className="post-page">
    ...
    {mode === "overview" ? <section className="post-overview">
      <div className="post-overview-hero"><div><p className="eyebrow">ONE MASTER · TWO CHANNELS</p><h2>先建立母版，再分发双频道</h2><p>一条 Picture Master 保持画面锁定；aBin Vision 使用道路声与授权音乐，aBin Ambience 只保留真实环境声。</p></div><dl><div><dt>工作区</dt><dd>{workflow.stages.length}</dd></div><div><dt>任务</dt><dd>{workflow.stages.reduce((sum, stage) => sum + stage.tasks.length, 0)}</dd></div><div><dt>调色起点</dt><dd>{workflow.gradePresets.length}</dd></div><div><dt>交付</dt><dd>4K HDR</dd></div></dl></div>
      <div className="post-overview-grid"><section><header><small>RECOMMENDED PATH</small><h3>推荐执行顺序</h3></header><ol><li><span>01</span><div><strong>确认项目上下文</strong><p>选择拍摄计划或建立独立项目，先锁定路线、帧率和输出目标。</p></div></li><li><span>02</span><div><strong>按工作区推进</strong><p>从 Media 到 Deliver 依序处理，不在调色前跳过素材整理与锁画。</p></div></li><li><span>03</span><div><strong>匹配后再套风格</strong><p>先逐镜匹配曝光与白平衡，再把五套预设作为起点。</p></div></li><li><span>04</span><div><strong>双频道质检交付</strong><p>核对 Vision 与 Ambience 画面同步、响度、HDR 元数据和上传结果。</p></div></li></ol><div className="post-overview-links"><button onClick={() => setMode("guide")}>查看工作区指南</button><button onClick={() => setMode("tutorial")}>从新手教程开始</button></div></section>
      <aside>{postTasks.length ? <><small>CURRENT PROJECT</small><h3>{postProject?.title ?? "当前后期项目"}</h3><div className="overview-progress"><strong>{progress}%</strong><span><i><b style={{ width: `${progress}%` }} /></i>{completed} / {postTasks.length} 项完成</span></div>{projectRoute && <p>{projectRoute.route.name}<br />{projectRoute.waypoints.map((point) => point.name).join(" · ")}</p>}<button className="overview-primary" onClick={() => setMode("pipeline")}><Play size={14} /> 继续项目执行</button><button className="overview-reset" onClick={clearPostWorkflow}><RotateCcw size={13} /> 重置当前流程</button></> : <><small>START A PROJECT</small><h3>建立后期执行清单</h3><label><CalendarDays size={15} /><span><small>关联拍摄计划</small><select value={selectedPlanId} onChange={(event) => setSelectedPlanId(event.target.value)}><option value="">独立后期项目</option>{plans.map((plan) => { const route = routes.find((item) => item.route.id === plan.routeId); return <option key={plan.id} value={plan.id}>{plan.scheduledDate} · {route?.route.name ?? plan.objective}</option>; })}</select></span></label><p>{selectedPlan ? "将携带路线、地点、拍摄方式和设备参数。" : plans.length ? "也可以建立不关联拍摄计划的独立项目。" : "还没有拍摄计划，可先独立建立，之后再补充项目资料。"}</p><button className="overview-primary" onClick={importWorkflow}><ListChecks size={14} /> 导入完整执行清单</button></>}</aside></div>
    </section> : mode === "guide" ? <section className="post-category">
      ...
      <div className="post-progress"><div><span><strong>{progress}%</strong> 已完成</span><small>{completed} / {postTasks.length || workflow.stages.reduce((sum, stage) => sum + stage.tasks.length, 0)} 项</small></div><i><b style={{ width: `${progress}%` }} /></i>{!postTasks.length && <button onClick={() => setMode("overview")}>先建立项目</button>}</div>
      <section className="post-master-status"><article className={stageIsDone("edit") ? "done" : ""}><small>PICTURE MASTER</small><strong>画面母版</strong><span>{stageIsDone("edit") ? "已锁画" : "等待 Edit 工作区锁画"}</span></article><article className={stageIsDone("fairlight") ? "done" : ""}><small>aBin VISION</small><strong>道路声 + 授权音乐</strong><span>{stageIsDone("fairlight") ? "音频结构已完成" : "由锁定母版同步派生"}</span></article><article className={stageIsDone("fairlight") ? "done" : ""}><small>aBin AMBIENCE</small><strong>真实环境声</strong><span>{stageIsDone("fairlight") ? "无音乐版本已完成" : "保持相同画面结构"}</span></article><article className={stageIsDone("deliver") ? "done" : ""}><small>4K HDR DELIVERY</small><strong>双频道交付</strong><span>{stageIsDone("deliver") ? "等待上传复核" : "HEVC Main10 · PQ"}</span></article></section>
      <div className="pipeline-focus-head"><div><small>ACTIVE WORKSPACE</small><h2>{selected.label}</h2><p>一次只完成一个工作区。勾完当前检查项并确认完成标准，再进入下一步。</p></div><span><strong>{selectedStageCompleted}</strong> / {selected.tasks.length}</span></div>
      {(() => { const stage = selected; const Icon = stageIcons[stage.id]; return <article id={`post-${stage.id}`} className={`pipeline-stage pipeline-stage-focus stage-${stage.id}`}><header><span><Icon size={19} /></span><div><small>STEP {String(selectedStageIndex + 1).padStart(2, "0")} · {stage.englishLabel}</small><h2>{stage.label}</h2><p>{stage.summary}</p></div><strong>{selectedStageCompleted}/{stage.tasks.length}</strong></header><div>{stage.tasks.map((title, index) => { const localTask = selectedStageTasks[index]; return <button key={title} className={localTask?.completed ? "done" : ""} onClick={() => localTask && togglePostTask(localTask.id)} disabled={!localTask}><span>{localTask?.completed ? <Check size={14} /> : index + 1}</span>{title}</button>; })}</div><footer>完成标准 · {stage.output}</footer></article>; })()}
      <nav className="pipeline-step-nav"><button disabled={selectedStageIndex <= 0} onClick={() => { const previous = workflow.stages[selectedStageIndex - 1]; if (previous) setSelectedId(previous.id); }}><ArrowLeft size={14} /> 上一工作区</button><div><small>下一步</small><strong>{workflow.stages[selectedStageIndex + 1]?.label ?? "全部工作区已完成"}</strong></div><button disabled={selectedStageIndex >= workflow.stages.length - 1} onClick={() => { const next = workflow.stages[selectedStageIndex + 1]; if (next) setSelectedId(next.id); }}>下一工作区 <ArrowRight size={14} /></button></nav>
    </section>}
  </main>;
}
