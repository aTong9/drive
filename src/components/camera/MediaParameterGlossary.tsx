...
export function MediaParameterGlossary({ initialQuery = "" }: { initialQuery?: string }) {
  ...
  return <section className="media-glossary">
    ...
    <div className="media-glossary-toolbar"><div role="tablist" aria-label="参数领域"><button role="tab" aria-selected={domain === "video"} className={domain === "video" ? "active" : ""} onClick={() => setDomain("video")}><Film size={16} />视频行业</button><button role="tab" aria-selected={domain === "audio"} className={domain === "audio" ? "active" : ""} onClick={() => setDomain("audio")}><Headphones size={16} />音频行业</button></div><label><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索 4K、HDR10、HLG、10-bit、LUFS、48 kHz…" /></label></div>
