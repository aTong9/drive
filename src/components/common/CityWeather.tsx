...
export function CityWeather({ cities, compact = false }: { cities: string[]; compact?: boolean }) {
  ...
  return <section className={`city-weather ${compact ? "is-compact" : ""}`} aria-label={`${city}天气预报`}>
    ...
    {status === "loading" && <div className="weather-loading" role="status" aria-live="polite"><i />正在获取天气预报…</div>}
    {status === "error" && <div className="weather-error" role="status">暂时无法取得天气，出发前请重新确认</div>}
